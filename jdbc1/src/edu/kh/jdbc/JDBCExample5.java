...
public class JDBCExample5 {
	...
	public static void main(String[] args) {
		...
		// 아이디, 비밀번호, 이름을 입력받아
		// TB_USER 테이블에 삽임(INSERT)하기

		/*
		 * java.sql.PreparedStatement - SQL 중간에 ? (위치홀더, placeholder)를 작성하여 ? 자리에 java
		 * 값을 대입할 준비가 되어있는 STatement
		 * 
		 * 장점 1 : SQL 작성이 간편해짐 장점 2 : ? 에 값 대입 시 자료형 맞는 형태의 리터럴으로 대입됨! ex) String 대입 ->
		 * '값' (자동으로 '' 추가) ex) int 대입 -> 값 장점 3 : 성능, 속도에서ㅏ 우위를 가지고 있음
		 * 
		 * *** PreaparedStatement는 Statement 자식이다 ***
		 * 
		 * 위치홀더(?)가
		 * 있을 때 : 무조건 PreparedStatement
		 * 없을 때 : Statement , PreparedStatement
		 */
		...
		try {
			...
			// 2. DeiverManager 를 이용해 Connection 객체 생성
			Class.forName("oracle.jdbc.driver.OracleDriver");

			String userName = "kh_hyg";
		...
		*/
			...
			// pstmt(prepareStatement) 에서 executeQuery(), executeUpdate() 
			// 매개변수 자리에 sql 들어오면 안됨
			int result = pstmt.executeUpdate();
			
			// 7. result 값에 따른 결과 처리 + 트랜잭션 제어처리
			if(result > 0) { // INSERT 성공 시
				conn.commit(); // COMMIT 수행 -> DB에 INSERT 데이터 양구 반영
