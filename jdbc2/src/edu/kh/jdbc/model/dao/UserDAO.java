...
// (Model 증 하나) DAO (Data Access Object : 데이터 접근 객체)
...
public class UserDAO {
	...
	public int insertUser(Connection conn, User user) throws Exception {
		...
		try {
			...
			// 6. 사용한 JDBC 객체 자원 반환
			// import static 사용해도되고 아니면 Class.메서드()로 호출 자유.
			// 난 헷갈리지 않게 메서드로 호출할듯.
			close(pstmt);
	...
	public List<User> selectName(Connection conn, String keyword) throws Exception {
		...
		String sql = "SELECT USER_NO, USER_ID, USER_PW, USER_NAME, ENROLL_DATE" + " FROM TB_USER"
	...
	/** 6.
	 *  6-1. 썜이 하신 풀이는 USER_NO로 조회했을 경우다.
	 * @param conn
	 * @param user
	 * @return
	 */
